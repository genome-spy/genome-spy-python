...
- Added upstream links across Getting started, all user-guide pages, the landing
  ...
  every row.
- Decided against upstream links on gallery example pages. A prototype that
  carried them in `META["upstream"]` and rendered a credit line on each
  generated detail page was reverted; gallery pages stay link-free. Any future
  attempt must go through example metadata, because `docs/gallery/*.md` is
