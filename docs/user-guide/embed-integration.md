# Advanced embed integration

Use this page when connecting GenomeSpy to a custom Python host. For notebook workflows, start with [Use chart selections in Python](embed-api.md).

The experimental wrapper supports top-level parameters, selection reads,
subscriptions and clearing, scoped view parameters, and record dataset updates. Scales,
mark events, binary dataset loading, and exports are not wrapped yet.

For a selection declared inside a named track, resolve that view first:
...
Only `params` is currently exposed on Python view handles.
