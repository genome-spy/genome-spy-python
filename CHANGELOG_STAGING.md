...
- Refreshed gallery thumbnails and corrected oncoprint, BAM, link-mark, and
  LUAD oncoprint layouts for complete, usable visualizations.
- Added Altair BSD-3-Clause attribution and license materials to source files,
  documentation, package metadata, and built distributions.
- Removed tabular data previews from documentation gallery example pages.
- Fixed invalid CI workflow YAML in the wheel license-metadata check.
- Fixed nonexistent setup-uv action versions across all GitHub workflows.
- Reduced package CI to package-focused tests and removed AlphaGenome-only test
  modules, while keeping codegen tests in their dedicated job.

## 2026-08-20

- Fixed oncoprint gene ordering so genes altered in the same number of samples
  keep a stable, reproducible order across platforms.
- Added the packaging metadata for publishing to PyPI: MIT license, authors,
  keywords, classifiers, and project URLs.

## 2026-08-19

- Fixed documentation site rendering after the Furo switch: the landing page
  regained its navigation and search, the gallery no longer repeats its example
  list below the cards, and card thumbnails fill the full card width.

## 2026-08-18

- Added links to the upstream GenomeSpy documentation across the user guide,
  getting started, landing, About, and API reference pages.
- Restructured the API reference into a grouped index with one page per public
  object, replacing the single-page member dump.
